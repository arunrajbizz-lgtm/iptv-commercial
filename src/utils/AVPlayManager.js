class AVPlayManager {
  constructor() {
    this.player = (window.webapis && window.webapis.avplay) ? window.webapis.avplay : null;
    this.container = null;
    this.initialized = !!this.player;
    this.currentUrl = "";
    this.retryAttempted = false;
  }

  // INIT
  async initialize(containerId) {
    try {
      this.container = document.getElementById(containerId);
      
      // TIZEN
      if (window.webapis && window.webapis.avplay) {
        this.player = window.webapis.avplay;
        this.initialized = true;
        console.log("AVPlay Ready");
        return true;
      }

      // HTML5 FALLBACK
      console.log("Fallback Video Player Ready");
      this.initialized = true;
      return true;
    } catch (error) {
      console.log("AVPlay Init Error", error);
      return false;
    }
  }

  switchProtocol(url) {
    if (url.startsWith("http://")) {
      return url.replace("http://", "https://");
    }
    if (url.startsWith("https://")) {
      return url.replace("https://", "http://");
    }
    return url;
  }

  // PLAY
  async play(url, retry = true) {
    try {
      if (!url) {
        throw new Error("Invalid URL");
      }

      this.currentUrl = url;
      this.retryAttempted = !retry;

      // AVPLAY
      if (this.player) {
        try {
          // Strict stop/close cycle from backup
          this.player.stop();
          this.player.close();
        } catch {
          // Ignore errors during initial cleanup
        }

        // OPEN
        console.log("AVPlay Opening:", url);
        this.player.open(url);

        // --- PROPERTIES MUST BE SET AFTER OPEN BUT BEFORE PREPARE ---
        
        // DISPLAY METHOD (Strictly "Default" as per backup)
        try {
          this.player.setDisplayMethod("PLAYER_DISPLAY_MODE_AUTO_ASPECT_RATIO");
        } catch (e) { console.log("Set DisplayMethod error", e); }

        // ADAPTIVE BITRATE SETTINGS (From backup)
        try {
          this.player.setStreamingProperty("ADAPTIVE_INFO", "BITRATES=2000~100000|STARTBITRATE=LOWEST");
          
          const isHLS = url.includes(".m3u8");
          const isTS = url.includes("/live/") || url.endsWith(".ts");
          if (isHLS || isTS) {
            this.player.setStreamingProperty("IS_LIVE", "TRUE");
          }
        } catch (e) {
          console.log("Set Streaming Property Error:", e);
        }

        // DISPLAY RECT
        let rect = { left: 0, top: 0, width: 1920, height: 1080 };
        this.player.setDisplayRect(rect.left, rect.top, rect.width, rect.height);

        // LISTENER
        this.player.setListener({
          onbufferingstart: () => console.log("Buffering Start"),
          onbufferingprogress: (percent) => console.log("Buffer Progress:", percent),
          onbufferingcomplete: () => console.log("Buffer Complete"),
          onstreamcompleted: () => {
            console.log("Stream End");
            // Auto-restart if live, as seen in backup logic
            if (url.includes("/live/")) this.play(url);
          },
          onerror: (error) => {
            console.error("AVPlay Error Event:", error);
            this.handleError(url);
          },
          oncurrentplaytime: (ms) => {
            // Can be used for progress tracking
          }
        });

        // PREPARE
        this.player.prepareAsync(
          () => {
            console.log("AVPlay Prepared, starting playback");
            this.player.play();
          },
          (error) => {
            console.error("AVPlay Prepare Error:", error);
            this.handleError(url);
          }
        );

        return true;
      }

      // HTML5
      this.createHTML5Player(url);
      return true;
    } catch (error) {
      console.log("AVPlay Play Exception:", error);
      this.handleError(url);
      return false;
    }
  }

  handleError(url) {
    if (!this.retryAttempted) {
      const retryUrl = this.switchProtocol(url);
      console.log("Retrying playback with protocol switch:", retryUrl);
      this.play(retryUrl, false);
    }
  }

  // HTML5 / TIZEN OBJECT
  createHTML5Player(url) {
    if (!this.container) return;

    this.container.innerHTML = "";
    
    if (window.tizen) {
      // Use the <object> tag from the working backup
      const obj = document.createElement("object");
      obj.type = "application/avplayer";
      obj.style.width = "100%";
      obj.style.height = "100%";
      this.container.appendChild(obj);
      console.log("Tizen Player Object Created");
      return;
    }

    const video = document.createElement("video");
    video.src = url;
    video.autoplay = true;
    video.controls = false;
    video.playsInline = true;
    video.style.width = "100%";
    video.style.height = "100%";
    video.style.objectFit = "cover";

    this.container.appendChild(video);
    this.html5 = video;

    video.onerror = () => {
      console.log("HTML5 Video Error");
      if (!this.retryAttempted) {
        const retryUrl = this.switchProtocol(url);
        this.retryAttempted = true;
        video.src = retryUrl;
        video.play();
      }
    };
  }

  // PAUSE
  pause() {
    try {
      if (this.player) {
        this.player.pause();
      } else if (this.html5) {
        this.html5.pause();
      }
    } catch (error) {
      console.log(error);
    }
  }

  // RESUME
  resume() {
    try {
      if (this.player) {
        this.player.play();
      } else if (this.html5) {
        this.html5.play();
      }
    } catch (error) {
      console.log(error);
    }
  }

  // STOP
  stop() {
    try {
      if (this.player) {
        this.player.stop();
        this.player.close();
      }
      if (this.html5) {
        this.html5.pause();
        this.html5.src = "";
        this.html5.load();
      }
    } catch (error) {
      console.log(error);
    }
  }

  // SEEK
  seek(seconds) {
    try {
      if (this.player) {
        this.player.seekTo(seconds * 1000);
      } else if (this.html5) {
        this.html5.currentTime = seconds;
      }
    } catch (error) {
      console.log(error);
    }
  }

  // CURRENT TIME
  getCurrentTime() {
    try {
      if (this.player) {
        return Math.floor(this.player.getCurrentTime() / 1000);
      } else if (this.html5) {
        return Math.floor(this.html5.currentTime);
      }
    } catch (error) {
      console.log(error);
    }
    return 0;
  }

  // DURATION
  getDuration() {
    try {
      if (this.player) {
        return Math.floor(this.player.getDuration() / 1000);
      } else if (this.html5) {
        return Math.floor(this.html5.duration);
      }
    } catch (error) {
      console.log(error);
    }
    return 0;
  }
}

const avplayManager = new AVPlayManager();
export default avplayManager;
